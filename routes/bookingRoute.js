...
const PartBooking = require('../models/bookingModel');
...
router.post('/book/part', auth.verifyUser, (req, res) => {
    ...
    console.log(pid)
    PartBooking.findOne({ "product_id": pid, "user_id": req.result._id }).then((parts) => {


        if (parts == null) {
            PartBooking.find({}).then((data) => {
                ...
                Product.findOne({ "_id": pid }).then((data2) => {
                    if (data2 != null) {
                        ...
                        const booking = new PartBooking({ "product_id": pid, "quantity": quantity, "price": price, "booked_At": booked_At, "user_id": req.result._id });
...
router.get('/retrieve/myBookings', auth.verifyUser, (req, res) => {
    let query = PartBooking.find({ "user_id": req.result._id }).sort({ "bookedDate": -1 }).populate(
        {
            path: "product_id"
        }
    );
...
router.post('/delete/booking', auth.verifyUser, (req, res) => {
    ...
    let query = PartBooking.findOne({ "_id": pid });
    query.then((data) => {
        if (data != null) {
            PartBooking.deleteOne({ "_id": pid }).then((result2) => {
...
router.post('/update/booking', auth.verifyUser, (req, res) => {
    ...
    PartBooking.findOne({ "_id": pid }).then((data) => {
        if (data != null) {
            PartBooking.findOne({ "_id": pid }).then((data2) => {
                if (data2 != null) {
                    ...
                    Product.findOne({ "_id": product_id }).then((data3) => {
                        if (data3 != null) {
                            ...
                            PartBooking.updateOne({ "_id": pid }, { $set: { "quantity": qty, "price": price } }).then((result2) => {
